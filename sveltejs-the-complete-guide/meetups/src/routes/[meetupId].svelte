<script context="module">
  export function preload(page) {
    const meetupId = page.params.meetupId;
    return this.fetch(
      `https://meetups-svelte.firebaseio.com/meetups/${meetupId}.json`
    )
      .then(res => {
        if (!res.ok) {
          throw new Error("Fetching meetup failed. Please try again later.");
        }
        return res.json();
      })
      .then(meetup => {
        return { loadedMeetup: { ...meetup, id: meetupId } };
      })
      .catch(err => {
